...
(
    ...
    `user_version`     INT                   DEFAULT 1 COMMENT '用户资料版本号，用于缓存控制',
...
(
    ...
    `article_version`  INT                   DEFAULT 1 COMMENT '文章内容版本号，用于缓存控制',
    `create_time`      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`          TINYINT               DEFAULT 0 COMMENT '是否已删除（0:未删除,1:已删除）',
    `deleted_time`     DATETIME COMMENT '删除时间',
...
(
    ...
    `status`        TINYINT              DEFAULT 1 COMMENT '状态（0:禁用,1:正常）',
    `category_version`       INT                   DEFAULT 1 COMMENT '分类版本号，用于缓存控制',
    `create_time`   DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`   DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`       TINYINT              DEFAULT 0 COMMENT '是否已删除（0:未删除,1:已删除）',
    `deleted_time`  DATETIME COMMENT '删除时间',
...
(
    ...
    `status`        TINYINT              DEFAULT 1 COMMENT '状态（0:禁用,1:正常）',
    `tag_version`       INT                   DEFAULT 1 COMMENT '标签版本号，用于缓存控制',
    `create_time`   DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`   DATETIME    DEFAULT NULL COMMENT '更新时间',
...
(
    ...
    `comment_version`  INT               DEFAULT 1 COMMENT '评论版本号，用于缓存控制',
    `create_time`      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`          TINYINT           DEFAULT 0 COMMENT '是否已删除（0:未删除,1:已删除）',
    `deleted_time`     DATETIME COMMENT '删除时间',
...
(
    ...
    `pushed_status`       TINYINT           DEFAULT 0 COMMENT '是否已推送（0:未推送,1:已推送）',
...
(
    ...
    `user_id`            BIGINT   NOT NULL COMMENT '用户ID',
    `article_publish`    TINYINT           DEFAULT 1 COMMENT '是否接收文章发布通知',
    `article_like`       TINYINT           DEFAULT 1 COMMENT '是否接收文章点赞通知',
    `article_collection` TINYINT           DEFAULT 1 COMMENT '是否接收文章收藏通知',
    `article_comment`    TINYINT           DEFAULT 1 COMMENT '是否接收文章评论通知',
    `comment_reply`      TINYINT           DEFAULT 1 COMMENT '是否接收评论回复通知',
    `comment_like`       TINYINT           DEFAULT 1 COMMENT '是否接收评论点赞通知',
    `follow`             TINYINT           DEFAULT 1 COMMENT '是否接收关注通知',
    `message`            TINYINT           DEFAULT 1 COMMENT '是否接收私信通知',
    `report`             TINYINT           DEFAULT 1 COMMENT '是否接收举报处理通知',
    `feedback`           TINYINT           DEFAULT 1 COMMENT '是否接收反馈处理通知',
    `system`             TINYINT           DEFAULT 1 COMMENT '是否接收系统通知',
    `email_notification` TINYINT           DEFAULT 1 COMMENT '是否通过邮件接收通知',
    `site_notification`  TINYINT           DEFAULT 1 COMMENT '是否通过站内信接收通知',
    `create_time`        DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`        DATETIME          DEFAULT NULL COMMENT '更新时间',
    `deleted`            TINYINT  NOT NULL DEFAULT 0 COMMENT '是否已删除(0:未删除,1:已删除)',
...
-- 初始化分类数据
INSERT INTO `category` (`name`, `slug`, `parent_id`, `sort_order`, `description`, `status`)
VALUES ('技术', 'tech', 0, 1, '技术相关文章', 1),
...
-- 初始化标签数据
INSERT INTO `tag` (`name`, `slug`, `description`, `status`)
VALUES ('Java', 'java', 'Java 编程语言', 1),
